...
UP_motifs_sep_uno <- UP_motifs_sep_uno %>%
  mutate(Gene_Consensus = paste(Gene, Consensus, sep = "_"))

# prepare dataframe and separate by different terms
DOWN_motifs_sep <- DOWN_motifs %>%
  separate(Motif.Name, into = c("Gene", "Info"), sep = "\\(", extra = "merge") %>%
  mutate(Info = paste0("(", Info))

DOWN_motifs_sep <- DOWN_motifs_sep %>%
  separate(Info, into = c("Factor", "Descripcion", "GSE", "Homer"), sep = "/", extra = "merge")
DOWN_motifs_sep$Factor <- gsub("[()]", "", DOWN_motifs_sep$Factor)
DOWN_motifs_sep$GSE <- gsub("\\)", "", DOWN_motifs_sep$GSE)
DOWN_motifs_sep$GSE <- gsub("\\(", "", DOWN_motifs_sep$GSE)

# select only the first motif of the different factors
DOWN_motifs_sep_uno <- DOWN_motifs_sep %>%
  group_by(Factor) %>%  
  slice(1) %>%                  
  ungroup()

# create new column with consensous seq + gene name
DOWN_motifs_sep_uno <- DOWN_motifs_sep_uno %>%
  mutate(Gene_Consensus = paste(Gene, Consensus, sep = "_"))

# NO
# prepare dataframe and separate by different terms
NO_motifs_sep <- NO_motifs %>%
  separate(Motif.Name, into = c("Gene", "Info"), sep = "\\(", extra = "merge") %>%
  mutate(Info = paste0("(", Info))

NO_motifs_sep <- NO_motifs_sep %>%
  separate(Info, into = c("Factor", "Descripcion", "GSE", "Homer"), sep = "/", extra = "merge")
NO_motifs_sep$Factor <- gsub("[()]", "", NO_motifs_sep$Factor)
NO_motifs_sep$GSE <- gsub("\\)", "", NO_motifs_sep$GSE)
NO_motifs_sep$GSE <- gsub("\\(", "", NO_motifs_sep$GSE)

# select only the first motif of the different factors
NO_motifs_sep_uno <- NO_motifs_sep %>%
  group_by(Factor) %>%  
  slice(1) %>%                  
  ungroup()

# create new column with consensous seq + gene name
NO_motifs_sep_uno <-NO_motifs_sep_uno %>%
  mutate(Gene_Consensus = paste(Gene, Consensus, sep = "_"))
...
DOWN_motifs_sep_top <- head(DOWN_motifs_sep_uno[order(DOWN_motifs_sep_uno$P.value), ], 20)
UP_motifs_sep_uno_top <- head(UP_motifs_sep_uno[order(UP_motifs_sep_uno$P.value), ], 20)
NO__sep_uno_top <- head(NO_motifs_sep_uno[order(NO_motifs_sep_uno$P.value), ], 20)

pdf("plots/top10motifs_curated.pdf", width = 13, height = 10)
ggplot(UP_motifs_sep_uno_top, aes(x = reorder(Gene_Consensus, minuslog2_pvalue), y = minuslog2_pvalue,fill = minuslog2_pvalue)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.7), width = 0.5) +
  coord_flip() +
  labs(title = "-log2(P-values) of Top 20 Motifs in UP peaks", x = "Motif", y = "-log2(P-value)") +
  theme_minimal() +
  theme(
...
ggplot(DOWN_motifs_sep_top, aes(x = reorder(Gene_Consensus, minuslog2_pvalue), y = minuslog2_pvalue, fill = minuslog2_pvalue)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.7), width = 0.5) +
  coord_flip() +
...
ggplot(NO_motifs_sep_uno_top, aes(x = reorder(Gene_Consensus, minuslog2_pvalue), y = minuslog2_pvalue, fill = minuslog2_pvalue)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.7), width = 0.5) +
  coord_flip() +
